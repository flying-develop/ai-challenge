...
class QwenEmbedder(EmbeddingProvider):
    """Провайдер эмбеддингов через DashScope API (Qwen text-embedding-v4).

    Endpoint: POST https://dashscope-intl.aliyuncs.com/api/v1/services/embeddings/
              text-embedding/text-embedding
    Батч: до 10 текстов за запрос.
    Retry: 3 попытки с экспоненциальной задержкой (1s, 2s, 4s).

    Args:
        api_key:   DASHSCOPE_API_KEY или QWEN_API_KEY.
    ...
    def _embed_batch(self, batch: list[str]) -> list[list[float]]:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                # Явно запрашиваем незжатый ответ — избегаем IncompleteRead с gzip
        ...
        try:
            with urllib.request.urlopen(req, timeout=self._timeout, context=self._ssl_ctx) as resp:
                ...
                body = gzip.decompress(raw).decode("utf-8") if encoding == "gzip" else raw.decode("utf-8")
